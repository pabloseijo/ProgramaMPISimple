...
int main(int argc, char* argv[]) {
  int rank, size,step=1,metodoElegido=atoi(argv[3]);
    ...
    long iteraciones_totales_montecarlo=atol(argv[1]);
...
double i1(long iter){
    srand(getpid());
    long aciertos=0;
    double numeroE =  2.718281828459045235360;
    for(long i=0;i<iter;i++){
      double x = (double)rand()/RAND_MAX * 10.0-5.0;
        double y = (double)rand()/RAND_MAX;

        if(pow(numeroE,-(x*x))>y) aciertos++;
	//printf("%lf %lf ",numeroE, -(x*x));
	//printf("%lf ",pow(numeroE,-(x*x)));
	//printf("\n"); 
    }
    //printf("%ld",aciertos);
    return (double)aciertos*10/iter;
}
...
double i3(long iter){
    ...
    for (long i = 0; i < iter; i++) {
        ...
        double y = ((double)rand()/RAND_MAX)*2000;

        if (1.0/sqrt(1.0-x*x) > y) aciertos++;
    }
    return 4000.0*(double)aciertos/iter;
}
...
double i5(long iter){
    double res=0.0;
    for(long i=1;i<iter;i++){
      res+=((double)6/(double)i)/(double)i;
    };
    return res;
}
...
double i8(long iter){
    ...
    for (long k = 1; k < iter; k++) {
        long long num_factorial = factorial(4 * k);
        long long den_factorial = factorial(k);

        // Cambiamos el orden para dividir antes de multiplicar
        long double termino = (1103 + 26390 * k) / (double)(pow(396, 4 * k));
...
double i10(long iter){
    ...
    for (long i = 0; i < iter; i++) {
        ...
        double y = ((double)rand() / RAND_MAX) * 1000;

        if (((x+3)/ (sqrt(9-x*x))) > y) aciertos++;
    }
    // La integral es la proporción de aciertos respecto al número de iteraciones
    return (double)aciertos*6.0*1000/iter;
}
